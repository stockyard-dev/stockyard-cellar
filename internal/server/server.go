package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-cellar/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-cellar"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/bottles",s.handleList)
    s.mux.HandleFunc("POST /api/bottles",s.handleCreate)
    s.mux.HandleFunc("POST /api/bottles/{id}/drink",s.handleDrink)
    s.mux.HandleFunc("DELETE /api/bottles/{id}",s.handleDelete)
